...
async function createFormData(pdfPath: string) {
  const { fileFromPath } = await import("formdata-node/file-from-path");
  const { FormData } = await import("formdata-node");

  const formData = new FormData();
  const pdfFile = await fileFromPath(pdfPath, "application/pdf");
...
async function analyzeStorybookPdfOnly(
  pdfPath: string,
  storyId: string
  ...
  try {
    ...
    const response = await fetch(API_URL, {
      method: "POST",
      body: formData as any, // Type assertion for FormData compatibility
    });
...
async function findPdfFiles(): Promise<PdfFileInfo[]> {
  ...
  try {
    ...
    console.log(
      `📁 Found ${joyfulPdfFiles.length} PDF files in data-joyful folder`
    );
  } catch (error: any) {
...
function flattenAnalysisResult(result: AnalysisResult) {
  const flat = {
...
function resultsToCSV(results: AnalysisResult[]): string {
  ...
  const csvRows = flatResults.map((result) => {
    return headers
      .map((header) => escapeCsvValue((result as any)[header]))
      .join(",");
  });
...
async function main() {
  ...
  const jsonOutputPath = join(
    OUTPUT_DIR,
    `pdf-only-analysis-results-${timestamp}.json`
  );
  ...
  const csvOutputPath = join(
    OUTPUT_DIR,
    `pdf-only-analysis-results-${timestamp}.csv`
  );
  ...
  const withValidationErrors = results.filter(
  ...
  console.log("\n📊 BREAKDOWN BY SOURCE:");
  console.log(
    `Data-joyful folder: ${joyfulResults.length} processed (${
      joyfulResults.filter((r) => r.success).length
    } successful)`
  );
